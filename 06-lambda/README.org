...
Llega el momento de *probar el código*. Para ello, es necesario configurar *variables de entorno locales* antes de ejecutarlo, *sustituyendo los valores por los datos de conexión de tu servidor MySQL*. A continuación, será necesario lanzar el comando de ejecución.

En *Windows CMD*, ejecuta el siguiente código:
#+begin_src bash
...
Si estás en *Windows PowerShell*, ejecuta lo siguiente:
#+begin_src bash
  $env:DB_NAME="NOMBRE_BASE_DATOS"
  $env:RDS_PROXY_HOST="localhost"
  $env:USER_NAME="USUARIO_BASE_DATOS"
  $env:PASSWORD="PASS_BASE_DATOS"

  python lambda.py '{\"CustID\": \"13339\", \"Name\": \"Mi Nombre y Apellido\"}'
#+end_src

Por último, si estás en *Linux*:
#+begin_src bash
  set DB_NAME=NOMBRE_BASE_DATOS
  set RDS_PROXY_HOST=localhost
  set USER_NAME=USUARIO_BASE_DATOS
...
- *Additional configuration / Initial database name* - Utiliza el nombre ~lambdadb~. Asegúrate de *apuntar este dato* para utilizarlo en la configuración de las variables de entorno de la función Lambda
...
*Modifica el rol* de la función Lambda para que incluya dicha política.
...
** Modificación del handler de la función
Como puedes ver, el nombre del fichero que se proporciona es ~lambda.py~. Por ello, debemos modificar la configuración de la función Lambda para que haga referencia a dicho nombre. Esta tarea se realiza *modificando el nombre del handler*:
[[./imagenes/08-handler.png]]
