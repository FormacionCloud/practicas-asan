...
Por último, crearemos un *servicio* para que se ejecute VS Code Server siempre que se arranque la máquina virtual. Para ello, ejecuta este comando:

#+begin_src bash
  sudo systemctl enable --now code-server@ubuntu
#+end_src

El comando crea el servicio y lo *inicia* inmediatamente (opción ~--now~).
